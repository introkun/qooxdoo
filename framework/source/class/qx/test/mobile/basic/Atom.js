...
qx.Class.define("qx.test.mobile.basic.Atom",
{
  ...
  members :
  {
    ...
    testIcon : function()
    {
      ...
      this.assertEquals(atom.getIconWidget().getContainerElement().src, atom.getIcon());
      
      var image2URL = qx.util.ResourceManager.getInstance().toUri("qx/icon/Tango/48/places/folder-remote.png");
      
      atom.setIcon(image2URL);
      this.assertEquals(atom.getIcon(), image2URL);
      this.assertTrue(atom.getIconWidget().getContainerElement().src.indexOf(atom.getIcon()) != -1);
